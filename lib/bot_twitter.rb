...
########## action 1 - Il va dire bonjour ##########
def random_choice(handles)
    selection = handles.sample(5)
    return selection   
end

def say_bonjour(selection, client)
    client.update("Hello_world #{selection} ! @the_hacking_pro")
end

########## action 2 - Il va liker bonjour ##########
def like_bonjour(client)
    selection2 = client.search("#bonjour_monde", result_type: "recent").take(25)
    client.favorite(selection2)
end

########## action 3 - Il va follow bonjour ##########
def follow_bonjour(client)
    selection3 = client.search("#bonjour_monde", result_type: "recent").take(20).user.id
    client.follow(selection3)
end

########## action 4 - Il like et follow en live ##########




########## activation_processus ##########
def perform
    ...
    #selection = random_choice(handles)   (desactivated)
    #say_bonjour(selection, client)       (desactivated)
    #like_bonjour(client)                 (desactivated)
